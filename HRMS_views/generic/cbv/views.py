...
from HRMS.filters import FilterSet
from HRMS.group_by import group_by_queryset
from HRMS.HRMS_middlewares import _thread_locals
from HRMS_views import models
from HRMS_views.cbv_methods import (
    get_short_uuid,
    hx_request_required,
    paginator_qry,
    sortby,
...
from HRMS_views.forms import DynamicBulkUpdateForm, ToggleColumnForm
from HRMS_views.templatetags.generic_template_filters import getattribute


@method_decorator(hx_request_required, name="dispatch")
class HRMSListView(ListView):
    """
    HRMSListView
    """
    ...
    template_name: str = "generic/HRMS_list.html"
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        update_initial_cache(request, CACHE, HRMSListView)
        ...
        if request and self._saved_filters.get("field"):
            ...
            for group in context["groups"]:
                for instance in group["list"]:
                    instance.ordered_ids = ordered_ids
                    ordered_ids.append(instance.pk)
        CACHE.get(self.request.session.session_key + "cbv")[HRMSListView] = context
        from HRMS.urls import path, urlpatterns
    ...
    def export_data(self, *args, **kwargs):
        ...
        class HRMSListViewResorce(resources.ModelResource):
            """
        ...
        book_resource = HRMSListViewResorce()
...
class HRMSSectionView(TemplateView):
    """
    HRMS Template View
    """
    ...
    template_name = "generic/HRMS_section.html"
...
@method_decorator(hx_request_required, name="dispatch")
class HRMSDetailedView(DetailView):
    """
    HorillDetailedView
    """

    title = "Detailed View"
    template_name = "generic/HRMS_detailed_view.html"
    header: dict = {"title": "HRMS", "subtitle": "HRMS Detailed View"}
    body: list = []
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        request = getattr(_thread_locals, "request", None)
        self.request = request
        update_initial_cache(request, CACHE, HRMSDetailedView)
...
@method_decorator(hx_request_required, name="dispatch")
class HRMSTabView(TemplateView):
    """
    HRMSTabView
    """

    view_id: str = get_short_uuid(3, "htv")
    template_name = "generic/HRMS_tabs.html"
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        request = getattr(_thread_locals, "request", None)
        self.request = request
        update_initial_cache(request, CACHE, HRMSTabView)
    ...
    def get_context_data(self, **kwargs):
        ...
        CACHE.get(self.request.session.session_key + "cbv")[HRMSTabView] = context

        return context


@method_decorator(hx_request_required, name="dispatch")
class HRMSCardView(ListView):
    """
    HRMSCardView
    """
    ...
    template_name = "generic/HRMS_card.html"
    context_object_name = "queryset"
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        request = getattr(_thread_locals, "request", None)
        self.request = request
        update_initial_cache(request, CACHE, HRMSCardView)
    ...
    def get_context_data(self, **kwargs):
        ...
        CACHE.get(self.request.session.session_key + "cbv")[HRMSCardView] = context
        referrer = self.request.GET.get("referrer", "")
        if referrer:
            # Remove the protocol and domain part
            referrer = "/" + "/".join(referrer.split("/")[3:])
...
class HRMSFormView(FormView):
    ...
    template_name = "generic/HRMS_form.html"
    ids_key: str = "instance_ids"
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        request = getattr(_thread_locals, "request", None)
        self.request = request
        update_initial_cache(request, CACHE, HRMSFormView)
    ...
    def get_form(self, form_class=None):
        ...
        CACHE.get(self.request.session.session_key + "cbv")[HRMSFormView] = form
        self.form = form
        return form


@method_decorator(hx_request_required, name="dispatch")
class HRMSNavView(TemplateView):
    """
    HRMSNavView
    ...
    template_name = "generic/HRMS_nav.html"
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        request = getattr(_thread_locals, "request", None)
        self.request = request
        update_initial_cache(request, CACHE, HRMSNavView)
    ...
    def get_context_data(self, **kwargs):
        ...
        CACHE.get(self.request.session.session_key + "cbv")[HRMSNavView] = context
        return context


@method_decorator(hx_request_required, name="dispatch")
class HRMSProfileView(DetailView):
    """
    GenericHRMSProfileView
    """

    template_name = "generic/HRMS_profile_view.html"
    view_id: str = None
    filter_class: FilterSet = None
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        request = getattr(_thread_locals, "request", None)
        self.request = request
        update_initial_cache(request, CACHE, HRMSProfileView)
