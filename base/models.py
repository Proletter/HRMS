...
from HRMS import HRMS_middlewares
...
class JobPosition(HRMSModel):
    ...
    objects = HRMSCompanyManager("department_id__company_id")
...
class JobRole(HRMSModel):
    ...
    objects = HRMSCompanyManager("job_position_id__department_id__company_id")
...
class RotatingWorkType(HRMSModel):
    """
    ...
    objects = HRMSCompanyManager("employee_id__employee_work_info__company_id")
...
class RotatingWorkTypeAssign(HRMSModel):
    """
...
class EmployeeShift(HRMSModel):
    ...
    objects = HRMSCompanyManager("employee_shift__company_id")
...
class EmployeeShiftSchedule(HRMSModel):
    """
    ...
    auto_punch_out_time = models.TimeField(
        ...
        help_text=_(
            "Time at which the HRMS will automatically check out the employee attendance if they forget."
        ),
    )
    company_id = models.ManyToManyField(Company, blank=True, verbose_name=_("Company"))

    objects = HRMSCompanyManager("shift_id__employee_shift__company_id")
...
class RotatingShiftAssign(HRMSModel):
    """
...
class WorkTypeRequest(HRMSModel):
    """
    ...
    def clean(self):
        request = getattr(HRMS_middlewares._thread_locals, "request", None)
...
class WorkTypeRequestComment(HRMSModel):
    """
...
class ShiftRequestComment(HRMSModel):
    """
...
class Tags(HRMSModel):
    title = models.CharField(max_length=30)
    color = models.CharField(max_length=30)
    company_id = models.ForeignKey(
        Company, null=True, editable=False, on_delete=models.PROTECT
    )
    objects = HRMSCompanyManager(related_company_field="company_id")

    def __str__(self):
        return self.title


class HRMSMailTemplate(HRMSModel):
...
class DynamicEmailConfiguration(HRMSModel):
    """
...
class MultipleApprovalCondition(HRMSModel):
...
class AnnouncementComment(HRMSModel):
    """
...
class DashboardEmployeeCharts(HRMSModel):
    from employee.models import Employee
...
class TrackLateComeEarlyOut(HRMSModel):
...
class PenaltyAccounts(HRMSModel):
    """
...
def create_deduction_cutleave_from_penalty(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        if apps.is_installed("payroll") and penalty_amount:
            Deduction = get_HRMS_model_class(app_label="payroll", model="Deduction")
            penalty = Deduction()
