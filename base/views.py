...
from HRMS.methods import get_HRMS_model_class
from HRMS_audit.forms import HistoryTrackingFieldsForm
from HRMS_audit.models import AccountBlockUnblock, AuditTag, HistoryTrackingFields
...
def initialize_database(request):
    ...
    if initialize_database_condition():
        if request.method == "POST":
            ...
            from HRMS.HRMS_settings import DB_INIT_PASSWORD as db_password
        ...
        return render(request, "initialize_database/HRMS_user.html")
    else:
        return redirect("/")
...
def initialize_database_user(request):
    ...
    if request.method == "POST":
        ...
        return render(
            request,
            "initialize_database/HRMS_company.html",
            {"form": CompanyForm(initial={"hq": True})},
        )
    return render(request, "initialize_database/HRMS_user_signup.html")
...
def initialize_database_company(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                "initialize_database/HRMS_department.html",
                {"form": DepartmentForm(initial={"company_id": company})},
            )
    return render(request, "initialize_database/HRMS_company.html", {"form": form})
...
def initialize_database_department(request):
    ...
    return render(
        request,
        "initialize_database/HRMS_department_form.html",
...
def initialize_database_job_position(request):
    ...
    if request.method == "POST":
        ...
        return render(
            request,
            "initialize_database/HRMS_job_position_form.html",
            {
                "form": form,
    ...
    return render(
        request,
        "initialize_database/HRMS_job_position.html",
...
class EmployeePasswordResetView(PasswordResetView):
    """
    HRMS View for Employee Reset Password
    """
...
def object_delete(request, obj_id, **kwargs):
    ...
    if apps.is_installed("pms") and redirect_path == "/pms/filter-key-result/":
        KeyResult = get_HRMS_model_class(app_label="pms", model="keyresult")
        ...
        if form.is_valid():
                        ...
                        <tr>
                            <td style="padding: 20px;">
                                ...
                                <b><p style="font-size: 14px;">Hi,<br>
                                    This email is being sent as part of mail sever testing from HRMS.</p></b>
            ...
            try:
                ...
                image_path = path.join(
                    settings.STATIC_ROOT, "images/ui/HRMS-logo.png"
                )
...
@login_required
@hx_request_required
@permission_required("base.change_HRMSmailtemplate")
def view_mail_template(request, obj_id):
    """
    This method is used to display the template/form to edit
    """
    template = HRMSMailTemplate.objects.get(id=obj_id)
...
@login_required
@require_http_methods(["POST"])
@permission_required("base.add_HRMSmailtemplate")
...
@login_required
@permission_required("base.delete_HRMSmailtemplate")
def delete_mail_templates(request):
    ids = request.GET.getlist("ids")
    result = HRMSMailTemplate.objects.filter(id__in=ids).delete()
...
def employee_shift_view(request):
    ...
    if apps.is_installed("attendance"):
        GraceTime = get_HRMS_model_class(app_label="attendance", model="gracetime")
...
def employee_permission_assign(request):
    ...
    installed_apps = [app for app in settings.INSTALLED_APPS if app in HRMS_apps]
...
def general_settings(request):
    ...
    if apps.is_installed("payroll"):
        PayrollSettings = get_HRMS_model_class(
            app_label="payroll", model="payrollsettings"
        )
        EncashmentGeneralSettings = get_HRMS_model_class(
...
@login_required
@permission_required("HRMS_audit.view_audittag")
...
@login_required
@hx_request_required
@permission_required("HRMS_audit.add_audittag")
...
@login_required
@hx_request_required
@permission_required("HRMS_audit.change_audittag")
...
@login_required
def get_HRMS_installed_apps(request):
