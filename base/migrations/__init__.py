...
try:
    ...
    HRMS_mail_templates = HRMSMailTemplate.objects.all()
...
try:
    ...
    for penalty in penalties:
        ...
        if not PenaltyAccounts.objects.filter(**filter_conditions).exists():
            HRMS = PenaltyAccounts(
                id=penalty.id,
                employee_id=penalty.employee_id,
                penalty_amount=penalty.penalty_amount,
            )
            if apps.is_installed("attendance"):
                HRMS.late_early_id = penalty.late_early_id
            if apps.is_installed("leave"):
                HRMS.leave_request_id = penalty.leave_request_id
                HRMS.leave_type_id = penalty.leave_type_id
                HRMS.minus_leaves = penalty.minus_leaves
                HRMS.deduct_from_carry_forward = penalty.deduct_from_carry_forward
            HRMS.save()
