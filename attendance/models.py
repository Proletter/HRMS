...
class AttendanceActivity(HRMSModel):
    """
...
class AttendanceRequestFile(HRMSModel):
    file = models.FileField(upload_to="attendance/request_files")


class AttendanceRequestComment(HRMSModel):
    """
...
class AttendanceOverTime(HRMSModel):
    """
...
class AttendanceLateComeEarlyOut(HRMSModel):
    """
...
class AttendanceValidationCondition(HRMSModel):
    """
...
class AttendanceGeneralSetting(HRMSModel):
    """
...
if apps.is_installed("leave") and apps.is_installed("payroll"):
    ...
    class PenaltyAccount(HRMSModel):
        """
    ...
    def create_initial_stage(sender, instance, created, **kwargs):
        ...
        if created:
            ...
            if penalty_amount:
                Deduction = get_HRMS_model_class(
...
class WorkRecords(models.Model):
    ...
    def title_message(self):
        ...
        if title_message == "Absent":
            if apps.is_installed("leave"):
                LeaveRequest = get_HRMS_model_class(
