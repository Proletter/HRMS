...
from HRMS.filters import HRMSPaginator
from HRMS.group_by import group_by_queryset
from HRMS.HRMS_settings import HRMS_DATE_FORMATS
from HRMS.methods import get_HRMS_model_class
from HRMS_audit.models import AccountBlockUnblock, HistoryTrackingFields
from HRMS_documents.forms import (
    DocumentForm,
    DocumentRejectForm,
    DocumentRequestForm,
    DocumentUpdateForm,
...
@login_required
@manager_can_enter("HRMS_documents.view_documentrequests")
def document_request_view(request):
    ...
    documents = filtersubordinates(
        request=request,
        perm="HRMS_documents.view_documentrequests",
        queryset=documents,
    )
...
@login_required
@hx_request_required
@manager_can_enter("HRMS_documents.add_documentrequests")
def document_request_create(request):
    ...
    form = choosesubordinates(request, form, "HRMS_documents.add_documentrequest")
    if request.method == "POST":
        form = DocumentRequestForm(request.POST)
        form = choosesubordinates(
            request, form, "HRMS_documents.add_documentrequest"
        )
        if form.is_valid():
            form = form.save()
...
@login_required
@hx_request_required
@manager_can_enter("HRMS_documents.change_documentrequests")
...
@login_required
@hx_request_required
@owner_can_enter("HRMS_documents.view_document", Employee)
...
@login_required
@hx_request_required
@owner_can_enter("HRMS_documents.add_document", Employee)
...
def document_delete(request, id):
    ...
    try:
        ...
        if not request.user.has_perm("HRMS_documents.delete_document"):
...
@login_required
@hx_request_required
@manager_can_enter("HRMS_documents.add_document")
...
def paginator_qry(qryset, page_number):
    ...
    """
    paginator = HRMSPaginator(qryset, get_pagination())
        ...
        for models in related_models:
            ...
            if field_name:
                ...
                if (
                    ...
                    Recruitment = get_HRMS_model_class(
                    ...
                    OnboardingStage = get_HRMS_model_class(
                    ...
                    OnboardingTask = get_HRMS_model_class(
    ...
    for column_value, column_name in selected_columns:
        ...
        for employee in employees:
            ...
            if type(value) == date:
                ...
                for format_name, format_string in HRMS_DATE_FORMATS.items():
...
def dashboard_employee_tiles(request):
    ...
    if apps.is_installed("recruitment"):
        Candidate = get_HRMS_model_class(app_label="recruitment", model="candidate")
...
def bonus_points_tab(request, emp_id):
    ...
    if apps.is_installed("payroll"):
        Reimbursement = get_HRMS_model_class(
