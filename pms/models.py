...
from HRMS_automations.methods.methods import get_model_class
from HRMS_views.cbv_methods import render_template

"""Objectives and key result section"""


class Period(HRMSModel):
...
class KeyResult(HRMSModel):
    ...
    history = HRMSAuditLog(bases=[HRMSAuditInfo])
...
class EmployeeObjective(HRMSModel):
    ...
    history = HRMSAuditLog(bases=[HRMSAuditInfo], related_name="history_set")
    archive = models.BooleanField(default=False)
...
class Comment(models.Model):
    ...
    history = HRMSAuditLog(excluded_fields=["comment"], bases=[HRMSAuditInfo])
    objects = HRMSCompanyManager(
...
class QuestionTemplate(HRMSModel):
...
class Question(HRMSModel):
    ...
    objects = HRMSCompanyManager("template_id__company_id")

    def __str__(self):
        return self.question


class QuestionOptions(HRMSModel):
    ...
    objects = HRMSCompanyManager("question_id__template_id__company_id")


class Feedback(HRMSModel):
