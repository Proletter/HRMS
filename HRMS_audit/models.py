...
from HRMS_audit.methods import remove_duplicate_history
...
class AuditTag(models.Model):
    ...
    class Meta:
        ...
        app_label = "HRMS_audit"


class HRMSAuditInfo(models.Model):
    """
    HRMSAuditInfo model to store additional info
    """
...
class HRMSAuditLog(HistoricalRecords):
    """
    Model to store additional information for historical records.
    """

    # def __init__(self, *args, bases=None, **kwargs):
    #     super(HRMSAuditLog, self).__init__(*args, **kwargs)
    #     self.is_HRMS_audit_log = True
...
def pre_create_HRMS_audit_log(sender, instance, *args, **kwargs):
    """
...
def post_create_HRMS_audit_log(sender, instance, *_args, **kwargs):
    """
    ...
    try:
        ...
        if isinstance(history_instance, HRMSAuditLog):
